...
export const validateEmail = (email: string): ValidationResult => {
  ...
  if (email.length > VALIDATION.EMAIL.MAX_LENGTH) {
    return {
      isValid: false,
      error: `Email must not exceed ${VALIDATION.EMAIL.MAX_LENGTH} characters`,
    };
  }
...
export const validateFullName = (name: string): ValidationResult => {
  ...
  if (trimmedName.length < VALIDATION.FULL_NAME.MIN_LENGTH) {
    return {
      isValid: false,
      error: `Name must be at least ${VALIDATION.FULL_NAME.MIN_LENGTH} characters`,
    };
  }
  ...
  if (trimmedName.length > VALIDATION.FULL_NAME.MAX_LENGTH) {
    return {
      isValid: false,
      error: `Name must not exceed ${VALIDATION.FULL_NAME.MAX_LENGTH} characters`,
    };
  }
...
export const validateProjectName = (name: string): ValidationResult => {
  ...
  if (trimmedName.length < VALIDATION.PROJECT_NAME.MIN_LENGTH) {
    return {
      isValid: false,
      error: `Project name must be at least ${VALIDATION.PROJECT_NAME.MIN_LENGTH} characters`,
    };
  }
  ...
  if (trimmedName.length > VALIDATION.PROJECT_NAME.MAX_LENGTH) {
    return {
      isValid: false,
      error: `Project name must not exceed ${VALIDATION.PROJECT_NAME.MAX_LENGTH} characters`,
    };
  }
...
export const validateProjectKey = (key: string): ValidationResult => {
  ...
  if (key.length < VALIDATION.PROJECT_KEY.MIN_LENGTH) {
    return {
      isValid: false,
      error: `Project key must be at least ${VALIDATION.PROJECT_KEY.MIN_LENGTH} characters`,
    };
  }
  ...
  if (key.length > VALIDATION.PROJECT_KEY.MAX_LENGTH) {
    return {
      isValid: false,
      error: `Project key must not exceed ${VALIDATION.PROJECT_KEY.MAX_LENGTH} characters`,
    };
  }
...
export const validateTaskTitle = (title: string): ValidationResult => {
  ...
  if (trimmedTitle.length < VALIDATION.TASK_TITLE.MIN_LENGTH) {
    return {
      isValid: false,
      error: `Task title must be at least ${VALIDATION.TASK_TITLE.MIN_LENGTH} characters`,
    };
  }
  ...
  if (trimmedTitle.length > VALIDATION.TASK_TITLE.MAX_LENGTH) {
    return {
      isValid: false,
      error: `Task title must not exceed ${VALIDATION.TASK_TITLE.MAX_LENGTH} characters`,
    };
  }
...
export const validateDescription = (description: string): ValidationResult => {
  ...
  if (description.length > VALIDATION.DESCRIPTION.MAX_LENGTH) {
    return {
      isValid: false,
      error: `Description must not exceed ${VALIDATION.DESCRIPTION.MAX_LENGTH} characters`,
    };
  }
...
export const validateTimeEntry = (
  ...
  if (endTime) {
    ...
    if (durationHours > 24) {
      return {
        isValid: false,
        error: 'Time entry duration cannot exceed 24 hours',
      };
    }
  }
