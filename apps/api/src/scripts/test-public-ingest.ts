...
async function testIngest() {
  ...
  logger.info(
    `Testing Public Inbound API at: ${API_URL}/ingest/${PROJECT_KEY}`
  );
  ...
  try {
    ...
    logger.info(
      `Attempting duplicate ingestion for External ID: ${externalId}`
    );
    try {
      await axios.post(
        `${API_URL}/ingest/${PROJECT_KEY}`,
      ...
      if (err.response?.status === 409) {
        logger.info(
          'Duplicate ingestion REJECTED as expected (409 Conflict):',
          err.response.data
        );
      } else {
        logger.error(
          'Duplicate ingestion failed with unexpected error:',
          err.message
        );
      }
    }
