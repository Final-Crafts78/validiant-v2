...
import { projects, projectTypes, typeColumns, records, recordHistory } from '../db/schema';
...
async function verify() {
  ...
  try {
    ...
    // 5. Cleanup
    await db.delete(recordHistory).where(eq(recordHistory.recordId, newRecord.id));
    ...
    fs.writeFileSync('service_verification.txt', 'Failed: ' + err.message + '\n' + log);
    process.exit(1);
  } finally {
    process.exit(0);
  }
