...
const STANDARD_FIELDS = [
  {
    ...
    label: 'Task Title',
    required: true,
    description: 'Main heading for the task',
  },
  {
    key: 'description',
    label: 'Description',
    required: false,
    description: 'Detailed context or instructions',
  },
  {
    key: 'candidateName',
    label: 'Candidate Name',
    required: false,
    description: 'Name of the person being verified',
  },
  {
    key: 'candidateEmail',
    label: 'Candidate Email',
    required: false,
    description: 'Contact email for BGV',
  },
  {
    key: 'externalId',
    label: 'External Reference ID',
    required: false,
    description: 'ID from your own system',
  },
  {
    key: 'address',
...
export function BulkUploadWizard({
  ...
  useEffect(() => {
    if (open) {
      logger.info('[BulkUpload:Mount]', {
        schemaFieldCount: fieldSchema.length,
  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        if (!sheet) {
          setParseError('Failed to read the sheet data.');
          return;
        }
        ...
        if (!firstRow) {
          setParseError('The sheet contains no data.');
          return;
        }
        ...
        [...STANDARD_FIELDS, ...fieldSchema].forEach((field) => {
          const key =
            'fieldKey' in field
              ? (field as VerificationField).fieldKey
  ...
  const mappedData = useMemo(() => {
    return rows.map((row) => {
      ...
      if (!result.title)
        result.title = `Bulk Task ${Date.now().toString().slice(-4)}`;
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span
                  className={`h-1.5 w-1.5 rounded-full ${step === 'upload' ? 'bg-indigo-500' : 'bg-slate-300'}`}
                />
                <span
                  className={`h-1.5 w-1.5 rounded-full ${step === 'map' ? 'bg-indigo-500' : 'bg-slate-300'}`}
                />
                <span
                  className={`h-1.5 w-1.5 rounded-full ${step === 'preview' ? 'bg-indigo-500' : 'bg-slate-300'}`}
                />
          ...
          {step === 'upload' && (
            <div
              ...
              onDrop={(e) => {
                ...
                if (f) {
                  // Manually trigger handleFileSelect logic by creating a synthetic event
                  // OR just call it directly with the file if we refactor.
                  // For now, type-cast to access the handler.
                  handleFileSelect({
                    target: { files: [f] },
            ...
            >
              ...
              <p className="text-slate-500 mt-2 max-w-sm">
                We support .CSV and .XLSX files. Our AI will attempt to auto-map
                your columns.
              </p>
              <input
                ref={fileInputRef}
              ...
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="bg-slate-100 px-4 py-1.5 rounded-full">
                  <p className="text-[10px] font-black text-slate-500">
                    COLUMNS DETECTED: {csvHeaders.length}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="space-y-3">
                  ...
                  {STANDARD_FIELDS.map((field) => (
                    <div
                      key={field.key}
                      className="flex items-center gap-4 bg-white border border-slate-200 p-4 rounded-2xl shadow-sm hover:border-indigo-200 transition-colors"
                    >
                ...
                {fieldSchema.length > 0 && (
                  <div className="space-y-3 pt-6">
                    <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest px-2">
                      Project Custom Data (Schema)
                    </p>
                    {fieldSchema.map((field) => (
                      <div
                        key={field.fieldKey}
                        className="flex items-center gap-4 bg-indigo-50/30 border border-indigo-100 p-4 rounded-2xl"
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {field.required && (
                              <span className="text-indigo-400 text-[10px] font-black underline decoration-indigo-200">
                                MANDATORY
                              </span>
                            )}
                          </div>
                          <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-widest opacity-60">
                            Type: {field.type}
                          </p>
                        </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-black text-slate-800 uppercase tracking-tight">
                    Data Preview (Bottom-Line Check)
                  </h3>
                </div>
                ...
                <table className="w-full text-left text-xs min-w-[600px]">
                  <thead>
                    <tr className="bg-slate-100 border-b border-slate-200">
                      <th className="p-4 font-black uppercase text-slate-500">
                        #
                      </th>
                      <th className="p-4 font-black uppercase text-slate-500">
                        Task Title
          ...
          {step === 'done' && (
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <p className="text-slate-500 mt-2 max-w-sm font-medium">
                We've successfully ingested {rows.length} records into the
                Command Center.
              </p>
            </div>
          )}
