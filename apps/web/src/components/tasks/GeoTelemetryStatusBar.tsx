  ...
  return (
    <div className="mb-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-500">
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center">
        ...
        {/* GPS Section */}
        <div className={`flex flex-1 items-center gap-3 p-4 transition-colors duration-500 ${
          isWithinRange 
            ? 'bg-emerald-50/50' 
            : !isTracking 
              ? 'bg-slate-50' 
          ...
          <div className={`flex h-10 w-10 items-center justify-center rounded-xl shadow-sm transition-transform duration-500 ${
            isWithinRange 
            ...
            {isWithinRange ? <MapPin className="h-5 w-5" /> : <LocateFixed className="h-5 w-5" />}
          </div>
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              {isTracking && (
                <span className={`flex h-1.5 w-1.5 rounded-full ${
                  accuracy && accuracy < 15 ? 'bg-emerald-500' : 'bg-amber-500'
                }`} />
              )}
            </div>
            <div className="flex items-center justify-between mt-0.5">
              <p className={`text-sm font-bold tracking-tight ${
                ...
                {!isTracking ? 'GPS DISCONNECTED' : isWithinRange ? 'WITHIN RANGE' : 'SEARCHING...'}
              </p>
        ...
        <div className="flex flex-1 items-center gap-3 p-4 bg-slate-50/30">
          <div className={`flex h-10 w-10 items-center justify-center rounded-xl transition-all ${
            elapsedTime > 0 ? 'bg-indigo-600 text-white shadow-indigo-100' : 'bg-slate-200 text-slate-400'
          }`}>
            <Clock className={`h-5 w-5 ${elapsedTime > 0 && 'animate-[spin_10s_linear_infinite]'}`} />
          </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between mt-0.5">
              ...
              <div className="flex items-center gap-1.5">
                <div className={`h-1.5 w-1.5 rounded-full ${elapsedTime > 0 ? 'bg-indigo-500 animate-pulse' : 'bg-slate-300'}`} />
