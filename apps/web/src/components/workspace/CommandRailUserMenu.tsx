...
import { Popover, PopoverTrigger, PopoverContent, Tooltip, TooltipTrigger, TooltipContent, TooltipProvider } from '@validiant/ui';
...
export function CommandRailUserMenu() {
  ...
  return (
    <Popover>
      ...
      <PopoverContent side="right" align="end" className="w-64 p-0 ml-2 overflow-hidden bg-glass border-[var(--color-border-base)]/30 shadow-obsidian rounded-2xl">
        <div className="p-4 bg-[var(--color-surface-subtle)]/50 border-b border-[var(--color-border-base)]/10">
          <div className="flex items-center gap-3">
             ...
             <div className="flex flex-col min-w-0">
               <span className="text-sm font-bold text-[var(--color-text-base)] truncate">{user.fullName}</span>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm transition-colors ${
                  isActive ? 'bg-[var(--color-accent-subtle)]/10 text-[var(--color-accent-base)] font-bold' : 'text-[var(--color-text-base)] hover:bg-[var(--color-surface-soft)]'
                }`}
              >
        ...
        <div className="p-2 space-y-1">
          <button onClick={() => activeOrg && router.push(`/${activeOrg.slug}/settings`)} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-[var(--color-text-base)] hover:bg-[var(--color-surface-soft)] transition-colors">
            <Settings className="w-4 h-4 text-[var(--color-text-muted)]" /> Settings
          </button>
          <button onClick={handleLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-red-500 hover:bg-red-500/10 transition-colors">
