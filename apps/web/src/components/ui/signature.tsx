...
export const Signature: React.FC<SignatureProps> = ({
  ...
  const stopDrawing = () => {
    ...
    if (dataUrl && dataUrl !== "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==") {
  ...
  return (
    <div className={cn("relative group overflow-hidden rounded-[2.5rem] bg-surface-lowest border border-white/5 p-8 shadow-obsidian-lg transition-all min-h-[250px]", className)}>
      {value ? (
        <div className="relative h-full w-full flex flex-col items-center justify-center gap-6">
          <img src={value} alt="Signature" className="max-h-[120px] grayscale contrast-125 transition-all group-hover:grayscale-0 duration-700 invert" />
          <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 rounded-full border border-emerald-500/20">
            <CheckCircle2 className="w-4 h-4 text-emerald-500" />
            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Legal Verification Locked</span>
          </div>
          <button
            onClick={() => onChange('')}
        ...
        <div className="flex flex-col h-full gap-6">
          ...
          <div className="text-center">
            <p className="text-[9px] font-bold text-white/10 uppercase tracking-widest">Sign within the capture boundary</p>
          </div>
        </div>
      )}
    </div>
