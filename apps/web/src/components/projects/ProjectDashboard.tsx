...
export function ProjectDashboard({ projectId }: ProjectDashboardProps) {
  ...
  const tableRows = tasks.slice(0, 10).map((task, i) => {
    ...
    if (
      ...
      statusBg = 'bg-primary';
      statusGlow =
        'shadow-[0_0_8px_rgba(var(--color-primary-rgb,173,198,255),0.8)]';
    ...
    <div className="w-full h-full overflow-y-auto no-scrollbar font-manrope">
      ...
      <div className="hidden lg:flex flex-col gap-8 w-full max-w-[1400px] mx-auto">
        ...
        <div className="bg-[var(--surface-container-low)] px-4 py-3 flex items-center justify-between rounded-xl">
          <div className="flex items-center gap-4">
            <input
              type="checkbox"
              className="rounded bg-transparent text-primary border-[var(--color-border-base)]"
            />
            ...
            <div className="flex gap-2">
              <button className="p-1.5 hover:bg-[var(--color-surface-muted)]/20 rounded text-[var(--color-text-muted)] transition-colors">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[var(--surface-container-low)]">
                  <th className="px-6 py-4 text-[10px] font-bold tracking-[0.15em] text-[var(--color-text-muted)] uppercase">
                    Status
                  </th>
                  <th className="px-6 py-4 text-[10px] font-bold tracking-[0.15em] text-[var(--color-text-muted)] uppercase">
                    Trace ID
              ...
              <tbody className="divide-y divide-[var(--color-border-base)]/10">
                {tableRows.map((row) => (
                  <tr
                    key={row.id}
                    className="hover:bg-[var(--surface-container-high)] transition-all duration-200 group relative"
                  >
                    <td className="px-6 py-4 group-hover:-translate-y-px transition-transform">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span
                          className={cn('text-xs font-bold', row.statusColor)}
                        >
                          {row.statusLabel}
                        </span>
                      </div>
                    ...
                    <td className="px-6 py-4 group-hover:-translate-y-px transition-transform">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-[var(--color-text-base)] max-w-[200px] truncate">
                          {row.entityName}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-xs font-mono text-[var(--color-text-muted)] group-hover:-translate-y-px transition-transform">
                      {row.latency}
                    </td>
                    <td className="px-6 py-4 text-xs font-medium text-[var(--color-text-muted)] group-hover:-translate-y-px transition-transform">
                      {row.timestamp}
                    </td>
                    <td className="px-6 py-4 group-hover:-translate-y-px transition-transform">
                      <div className="w-24 h-1.5 bg-[var(--surface-container-highest)] rounded-full overflow-hidden">
                        <div
                          className={cn('h-full rounded-full', row.confFill)}
                        />
                      </div>
                    </td>
          ...
          <div className="bg-[var(--surface-container-low)] p-6 rounded-2xl relative overflow-hidden group hover:-translate-y-1 transition-all duration-300 shadow-[0_12px_40px_rgba(15,23,42,0.02)]">
            ...
            <div className="relative z-10 space-y-4">
              <div className="flex justify-between items-start">
                <span className="text-[10px] font-black tracking-[0.2em] text-[var(--color-text-muted)] uppercase">
                  Completion Rate
                </span>
                <Activity className="text-primary w-5 h-5" />
              </div>
              <div className="flex items-baseline gap-2 font-headline">
                <span className="text-4xl font-black text-[var(--color-text-base)]">
                  {completionRate}
                </span>
          ...
          <div className="bg-[var(--surface-container-low)] p-6 rounded-2xl relative overflow-hidden group hover:-translate-y-1 transition-all duration-300 shadow-[0_12px_40px_rgba(15,23,42,0.02)]">
            ...
            <div className="relative z-10 space-y-4">
              ...
              <div className="text-[10px] font-medium text-[var(--color-text-muted)] flex items-center justify-between">
                ...
                <span className="text-[#ffb786] font-bold uppercase tracking-widest">
                  Optimized
                </span>
              </div>
            </div>
      ...
      <div className="lg:hidden flex flex-col gap-6 w-full pb-28">
        <header className="flex flex-col gap-4">
          <h2 className="text-3xl font-black text-[var(--color-text-base)] tracking-tighter">
            {projectName}
          </h2>
          ...
          <div className="flex items-center gap-3 overflow-x-auto no-scrollbar pb-2">
            <button className="whitespace-nowrap px-4 py-2 bg-primary/10 text-primary rounded-full text-[10px] font-black uppercase tracking-widest border border-primary/20">
              All Active
            </button>
            <button className="whitespace-nowrap px-4 py-2 bg-[var(--surface-container-highest)] text-[var(--color-text-base)] rounded-full text-[10px] font-bold uppercase tracking-widest">
              Critical
            </button>
            <button className="whitespace-nowrap px-4 py-2 bg-[var(--surface-container-highest)] text-[var(--color-text-base)] rounded-full text-[10px] font-bold uppercase tracking-widest">
              Near Me
        ...
        <section className="flex flex-col gap-4">
          {tasks.slice(0, 5).map((task) => (
            <div
              key={task.id}
              className="bg-[var(--surface-container-low)] p-5 rounded-2xl flex flex-col gap-4 border border-[var(--color-border-base)]/5 shadow-md"
            >
              <div className="flex justify-between items-start">
                <span className="text-[9px] font-black tracking-widest text-[var(--color-text-muted)] uppercase">
                  CASE: {task.id.substring(0, 8)}
                </span>
              ...
              <h3 className="text-xl font-black text-[var(--color-text-base)] leading-tight">
                {task.title}
              </h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-xs font-bold text-[var(--color-text-base)]">
                    Field Audit
                  </span>
                </div>
                <div className="flex flex-col gap-1">
              ...
              <div className="pt-2 flex justify-between items-center border-t border-[var(--color-border-base)]/10 mt-1">
                <div className="flex -space-x-2">
                  <div className="w-6 h-6 rounded-full bg-[var(--surface-container-high)] border border-[var(--surface-container-low)] flex items-center justify-center text-[8px] font-bold">
                    V
                  </div>
                  <div className="w-6 h-6 rounded-full bg-primary/20 text-primary border border-[var(--surface-container-low)] flex items-center justify-center text-[8px] font-bold">
                    +2
                  </div>
                </div>
        ...
        <section className="grid grid-cols-2 gap-4">
          <div className="bg-[var(--surface-container-low)] p-4 rounded-xl flex flex-col gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-2xl font-black text-[var(--color-text-base)] font-headline">
                12.4
              </span>
