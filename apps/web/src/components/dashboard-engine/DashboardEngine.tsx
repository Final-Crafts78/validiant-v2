...
export default function DashboardEngine({
    ...
    widgets: [
      {
        ...
        w: 12,
        h: 4,
        widgetId: 'project-status-matrix',
      },
  ...
  if (!mounted) {
    return (
      <div className="h-64 w-full animate-pulse bg-[var(--color-surface-subtle)] rounded-3xl" />
    );
  }
        ...
        <div className="fixed inset-0 z-50 flex justify-end bg-black/50 backdrop-blur-sm transition-opacity">
          <div className="w-full max-w-sm h-full bg-[var(--color-surface-base)] shadow-2xl p-6 flex flex-col border-l border-[var(--color-border-base)]/10 animate-in slide-in-from-right duration-300">
            <div className="flex justify-between items-center mb-6">
              <h2 className="font-bold text-lg text-[var(--color-text-base)]">
                Widget Catalog
              </h2>
              <button
                onClick={() => setIsAddWidgetOpen(false)}
                className="p-2 rounded-full hover:bg-[var(--color-surface-subtle)]"
              >
              ...
              {getAllWidgets().map((widget) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <h4 className="font-bold text-sm text-[var(--color-text-base)]">
                        {widget.name}
                      </h4>
          ...
          return (
            <div key={item.i}>
              ...
              >
                <Suspense
                  fallback={
                    <div className="h-full w-full flex items-center justify-center text-[var(--color-text-muted)] text-xs animate-pulse opacity-50">
                      Loading Widget...
                    </div>
                  }
                >
