...
export default function TimelineOverview({}: WidgetProps) {
  return (
        ...
        {milestones.map((m) => (
          <div key={m.id} className="relative z-10 flex flex-col items-center group">
            <div className={`w-4 h-4 rounded-full border-2 ${
              ...
              m.status === 'active' ? 'bg-[var(--color-surface-base)] border-primary shadow-[0_0_10px_var(--color-accent-base)]' :
              'bg-[var(--color-surface-base)] border-[var(--color-border-base)]'
            } transition-all`} />
            <div className="absolute top-6 w-max text-center">
              <span className="block text-xs font-bold text-[var(--color-text-base)]">{m.name}</span>
              <span className="block text-[10px] text-[var(--color-text-muted)] uppercase tracking-wider">{m.date}</span>
            </div>
          </div>
        ))}
      </div>
