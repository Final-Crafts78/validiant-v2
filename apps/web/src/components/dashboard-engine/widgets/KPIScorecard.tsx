...
export default function KPIScorecard(_props: WidgetProps) {
    ...
    <div className="flex h-full w-full">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full content-center">
        {metrics.map((m, i) => (
          <div
            key={i}
            className="flex flex-col bg-[var(--color-surface-subtle)] p-4 rounded-2xl border border-[var(--color-border-base)]/5"
          >
            ...
            <div className="flex items-end gap-3">
              ...
              <span
                className={`text-xs font-bold mb-1 ${m.trendUp ? 'text-green-500' : 'text-red-500'}`}
              >
                {m.trend}
              </span>
            </div>
