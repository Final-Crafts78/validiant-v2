...
import { listBackups, triggerBackup, getSystemHealth } from '@/services/infra.service';
      ...
      <div className="max-w-4xl mx-auto py-20 px-6">
        <div className="bg-white border border-slate-200 rounded-[2.5rem] p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 max-w-sm mx-auto mb-8 font-medium">
            You do not have the <code className="bg-slate-100 px-1.5 py-0.5 rounded text-red-600 text-xs font-bold">infra:backup</code> permission required to access this portal.
          </p>
          <button 
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Edge Network', status: health?.api || 'Operational', icon: Activity, color: 'text-emerald-600', bg: 'bg-emerald-50' },
          { label: 'Core Database', status: health?.database || 'Healthy', icon: Database, color: 'text-indigo-600', bg: 'bg-indigo-50' },
          { label: 'S3 Storage (R2)', status: health?.storage || 'Connected', icon: HardDrive, color: 'text-amber-600', bg: 'bg-amber-50' },
          { label: 'Compute Engine', status: health?.worker || 'Nominal', icon: Server, color: 'text-sky-600', bg: 'bg-sky-50' },
        ].map((stat, idx) => (
          <div key={idx} className="bg-white border border-slate-200 rounded-3xl p-5 flex items-center gap-4 shadow-sm hover:shadow-md transition-all">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-slate-800 capitalize leading-tight">{stat.status}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm">
        ...
        {backups.length === 0 ? (
          <div className="p-20 text-center flex flex-col items-center">
            ...
            <p className="text-sm font-bold text-slate-800">No restorable snapshots found</p>
            <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto leading-relaxed">
              Snapshots are automatically generated every 24 hours. You can also trigger a manual cold backup above.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-wider">Storage Key</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-wider">Footprint</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-wider">Created</th>
                  <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-wider text-right">Actions</th>
                </tr>
              </thead>
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Verified Integrity</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-slate-500">
                          {formatDistanceToNow(new Date(bk.uploadedAt), { addSuffix: true })}
                        </span>
                      </div>
                    </td>
      ...
      <div className="bg-amber-600 rounded-[2.5rem] p-8 text-white flex flex-col md:flex-row items-center gap-8 shadow-xl shadow-amber-100 relative group overflow-hidden">
        ...
        <div className="flex-1 relative z-10 text-center md:text-left">
          ...
          <p className="text-sm text-amber-50 leading-relaxed font-medium max-w-xl">
            Cold storage snapshots contain sensitive organization data. Accessing or downloading these artifacts is logged under the <span className="underline decoration-white/30">Forensic Audit Protocol</span>. Unauthorized restoration may trigger a security incident.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 relative z-10 bg-white/10 p-5 rounded-3xl border border-white/20">
          ...
          <div>
            <p className="text-sm font-black text-white leading-tight">ISO-27001 Compliant</p>
            <p className="text-[10px] text-amber-100 font-bold uppercase tracking-widest mt-0.5">Auditing Enabled</p>
          </div>
        </div>
      </div>
    </div>
